...
class RACE(torch.nn.Module):
    ...
    def forward(self, data: Dict[str, torch.Tensor], backprop: bool = True):
        ...
        Rij_result = get_edge_relative_vectors_with_pbc(R, data)
        if isinstance(Rij_result, tuple):
            Rij, shifts, displacement = Rij_result
        else:
            Rij = Rij_result
            num_graphs = data["ptr"].numel() - 1
            displacement = torch.zeros((num_graphs, 3, 3), dtype=R.dtype, device=R.device)

        Rij = Rij / self.cutoff
        #torch.set_printoptions(threshold=torch.inf)
        print("[DEBUG] Rij:", Rij.shape, "\n", Rij)
        ...
        species = data["species"].unsqueeze(-1)
        node_attrs = to_one_hot(species, self.num_species)
        print("[DEBUG] node_attrs (one-hot):", node_attrs.shape, "\n", node_attrs)

        node_feats = self.node_embedding(node_attrs)
        print("[DEBUG] node_feats after node_embedding:", node_feats.shape, "\n", node_feats)
        ...
        edge_attrs = self.spherical_harmonics(Rij)
        print("[DEBUG] edge_attrs (SH):", edge_attrs.shape, "\n", edge_attrs)

        edge_feats = self.radial_embedding(lengths.unsqueeze(1), node_attrs, data["edge_index"], species)
        print("[DEBUG] edge_feats (radial):", edge_feats.shape, "\n", edge_feats)

        x_node_feats = self.x_node_tp(node_feats, node_attrs)
        print("[DEBUG] x_node_feats (initial node prod):", x_node_feats.shape, "\n", x_node_feats)
        ...
        for i, (interaction, product, readout) in enumerate(zip(self.interactions, self.products, self.readouts)):
            ...
            print(f"[DEBUG] interaction {i}: node_feats = {node_feats.shape}, sample =\n", node_feats)
            print(f"[DEBUG] sc = {sc.shape}, sample =\n", sc)
            node_feats = product(
                x_node_feats=x_node_feats,
                node_feats=node_feats,
                sc=sc,
            )
            print(f"[DEBUG] product {i}: node_feats = {node_feats.shape}, sample =\n", node_feats)

            node_feats_list.append(node_feats)
            node_energies = readout(node_feats, node_heads)
            print(f"[DEBUG] readout {i}: node_energies = {node_energies.shape}, sample =\n", node_energies)

            outputs.append(node_energies[:, 0])
            if node_energies.shape[1] == 2:
                node_logvar_list.append(node_energies)

        node_energy = torch.stack(outputs, dim=-1)
        print("[DEBUG] node_energy (stacked):", node_energy.shape, "\n", node_energy)

        if self.act_fn is not None:
            node_energy = self.act_fn(node_energy)

        # wrapping vs training
        if hasattr(self, "training_mode_for_lammps") and self.training_mode_for_lammps:
            node_energy = torch.sum(node_energy, dim=-1)
            print("[DEBUG] node_energy sum (for LAMMPS):", node_energy.shape)
            print("[DEBUG] total energy (for LAMMPS):", torch.sum(node_energy).item())
            preds = {
                "node_energy": node_energy,
                "energy": node_energy.sum(dim=0, keepdim=True),
                "energy_var": torch.zeros(1, device=node_energy.device),
            }
        else:
            node_energy = torch.sum(node_energy, dim=-1)
            node_energy = scatter_sum(
            ...
            print("[DEBUG] final energy per structure:", node_energy.shape, "\n", node_energy)

            if node_logvar_list:
                node_logvar_tensor = torch.stack(node_logvar_list, dim=-1)
                node_logvar = node_logvar_tensor.mean(dim=-1)
            else:
                node_logvar = torch.zeros(node_feats.shape[0], device=node_energy.device)
            ...
            preds = {
                ...
                "node_energy": torch.zeros_like(node_energy),  # dummy
            }
        ...
        if self.regress_forces == 'direct' or self.regress_forces:
            ...
            if forces is None:
                raise RuntimeError("Error: The model did not return forces.")
            preds["forces"] = forces
            print("[DEBUG] forces:", forces.shape, "\n", preds["forces"])
...
def get_edge_relative_vectors_with_pbc(R: torch.Tensor, data_graph: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    iatoms = data_graph["edge_index"][0]  # shape = (b * n_edges)
    jatoms = data_graph["edge_index"][1]  # shape = (b * n_edges)
    Sij = data_graph["edges"]  # shape = (b * n_edges, 3), 필수 키로 가정
    cell = data_graph["cell"]  # shape = (b, 3, 3), 필수 키로 가정

    b, _, _ = cell.shape
    n_edges = iatoms.shape[0] // b  # num_edges를 edge_index에서 유추 (b * n_edges / b)
    ...
    Rij = _R + shift_v
    Rij = Rij.view(b * n_edges, 3).to(R.device)
    lengths = torch.norm(Rij, dim=1)  # 사용되지 않음, 필요 시 반환값에 추가

    return Rij  # (num_edges, 3)


def get_edge_relative_vectors_with_pbc_lammps(R: torch.Tensor, data: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    For LAMMPS export (wrapping용).
    Directly use shifts without re-computing them.
    """
    iatoms = data["edge_index"][0]
    jatoms = data["edge_index"][1]
    shifts = data["shifts"]
    print(' shift_v ==+--> ', shifts)
    cell = data["cell"]

    Rij = R[jatoms] - R[iatoms] + shifts

    displacement = torch.zeros((cell.shape[0], 3, 3), dtype=R.dtype, device=R.device)

    return Rij, shifts, displacement
