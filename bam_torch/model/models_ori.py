...
class RACE(torch.nn.Module):
    ...
    def forward(self, data: Dict[str, torch.Tensor], backprop: bool = True):
        ...
        node_energy = torch.stack(outputs, dim=-1)   # (64, 3)

        if self.act_fn is not None:
            node_energy = self.act_fn(node_energy)

        # ** wrapping vs training 구분 **
        if hasattr(self, "training_mode_for_lammps") and self.training_mode_for_lammps:
            # --- LAMMPS Wrapping ---
            node_energy = torch.sum(node_energy, dim=-1)
            print("[DEBUG] node_energy shape:", node_energy.shape,
                   " first few values:", node_energy[:5])
            print("[DEBUG] energy shape:", torch.sum(node_energy).shape,
                   " value:", torch.sum(node_energy).item())
            preds = {
                "node_energy": node_energy,  # (n_atoms,)
                "energy": node_energy.sum(dim=0, keepdim=True),  # (1,) scalar
        ...
        if self.regress_forces == 'direct' or self.regress_forces:
            ...
            if forces is None:
                raise RuntimeError("Error: The model did not return forces, but forces are required for MD simulations.")
            else:
                preds["forces"] = forces
