...
void PairMACE::compute(int eflag, int vflag) {
  ...
  cell[0][0] = domain->h[0]; cell[0][1] = 0.0; cell[0][2] = 0.0;
  cell[1][0] = domain->h[5]; cell[1][1] = domain->h[1]; cell[1][2] = 0.0;
  cell[2][0] = domain->h[4]; cell[2][1] = domain->h[3]; cell[2][2] = domain->h[2];
  ...
  for (int ii=0; ii<n_nodes; ++ii) {
    ...
    for (int jj=0; jj<jnum; ++jj) {
      ...
      if (rsq < r_max_squared) {n_edges++; n_edges_vec[ii]++;}
    }
  }

  std::vector<int> first_edge(n_nodes);
  first_edge[0] = 0;
  for (int ii=0; ii<n_nodes-1; ++ii) first_edge[ii+1] = first_edge[ii] + n_edges_vec[ii];
  ...
  for (int ii=0; ii<n_nodes; ++ii) {
    ...
    for (int jj=0; jj<jnum; ++jj) {
      ...
      if (rsq < r_max_squared) {
        edge_index[0][k] = i;
        edge_index[1][k] = domain_decomposition ? j : atom->map(atom->tag[j]);
        edges[k][0] = delx; edges[k][1] = dely; edges[k][2] = delz; // ★ 추가
        k++;
      }
    }
  }
  ...
  std::cout << edges.slice(0, 0, 5) << std::endl;
